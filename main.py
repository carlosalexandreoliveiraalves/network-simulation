...
from functions import choose_rip_algorithm, choose_rip_end_algorithm
from functions import choose_ospf_algorithm, choose_ospf_end_algorithm
...
def draw():
    ...
    flag = py.confirm(text='Escolha um algoritmo: ', title='Algoritmo', buttons=["1 - OSPF", "2 - RIP", "3 - OSPF_END", "4 - RIP_END"])
    ...
    if flag == "1 - OSPF":
        ...
        used_algo = "RIP"
    elif flag == "3 - OSPF_END":
        try:
            end_node = py.confirm(text='Em qual nó deseja terminar?', title='Algoritmo', buttons=optionsNodes)
        except IndexError:
            py.alert(text="Não há mais nós a serem mostrados", title='Fim', button='OK')
            os._exit(1)
        combined_path = choose_ospf_end_algorithm(current_node, visited_nodes, all_nodes, graph, end_node)
        used_algo = "OSPF END"
    elif flag == "4 - RIP_END":
        try:
            end_node = py.confirm(text='Em qual nó deseja terminar?', title='Algoritmo', buttons=optionsNodes)
            ...
            py.alert(text="Não há mais nós a serem mostrados", title='Fim', button='OK')
            os._exit(1)
        combined_path = choose_rip_end_algorithm(current_node, visited_nodes, all_nodes, graph, end_node)
        used_algo = "RIP END"
